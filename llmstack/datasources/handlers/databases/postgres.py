...
from llmstack.common.blocks.data.store.postgres.read import PostgresReader
from llmstack.common.blocks.data.store.postgres.read import PostgresReaderInput
...
class PostgresDataSource(DataSourceProcessor[PostgresDatabaseSchema]):
    ...
    def similarity_search(self, query: str, **kwargs) -> List[dict]:
        ...
        result = pg_client.process(
            PostgresReaderInput(
            sql=query,
            ), configuration=self._reader_configuration,
        ).documents[0].content_text
